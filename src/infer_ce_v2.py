...
infer_ce.py'nin v2 karsiligi; v1 ciktilarinin uzerine YAZMAZ.
Her fold icin artifacts/ce_v2_test_fold{i}.npy yazilir (varsa atlanir).
Sonda 3 foldun ortalamasi + proxy-sweep esigiyle v2 submission uretilir.

Kullanim: python infer_ce_v2.py [threshold]
"""
import sys
import time
...
def main():
    ...
    for fold in range(C.CE_N_FOLDS):
        path = C.ARTIFACTS_DIR / f"ce_v2_test_fold{fold}.npy"
        if path.exists():
        ...
        model.load_state_dict(torch.load(C.MODELS_DIR / f"ce_v2_fold{fold}.pt",
    ...
    np.save(C.ARTIFACTS_DIR / "ce_v2_test_mean.npy", mean_scores)

    if thr_arg is None:
        pz = np.load(C.ARTIFACTS_DIR / "ce_v2_proxy_scores.npz")
    ...
    out = C.OUTPUT_DIR / f"sub_v2_ce_thr{thr:.2f}.csv"
