"""Faz 5: CE v2 fold modelleriyle 3.36M test ciftini skorla (resumable).

infer_ce.py'nin v3 karsiligi; v1 ciktilarinin uzerine YAZMAZ.
Her fold icin artifacts/ce_v5_test_fold{i}.npy yazilir (varsa atlanir).
Sonda 3 foldun ortalamasi + proxy-sweep esigiyle v2 submission uretilir.

Kullanim: python infer_ce_v5.py [threshold]
"""
import sys
import time
...
def main():
    ...
    for fold in range(C.CE_N_FOLDS):
        path = C.ARTIFACTS_DIR / f"ce_v5_test_fold{fold}.npy"
        if path.exists():
        ...
        model.load_state_dict(torch.load(C.MODELS_DIR / f"ce_v5_fold{fold}.pt",
    ...
    np.save(C.ARTIFACTS_DIR / "ce_v5_test_mean.npy", mean_scores)

    if thr_arg is None:
        pz = np.load(C.ARTIFACTS_DIR / "ce_v5_proxy_scores.npz")
    ...
    out = C.OUTPUT_DIR / f"sub_v5_ce_thr{thr:.2f}.csv"
