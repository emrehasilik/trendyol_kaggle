"""Faz 5: cross-encoder v2 egitimi (pseudo-label + CE-onayli hard negatifler).
...
- Girdi : artifacts/train_dataset_v2.parquet
- Cikti : models/ce_v2_fold{i}.pt, artifacts/ce_v2_oof.npz,
          artifacts/ce_v2_proxy_scores.npz
...
def train_fold_v2(fold, tr_idx, data, cache):
    t_rows, i_rows, labels = data
    ckpt = C.MODELS_DIR / f"ce_v2_fold{fold}.pt"
...
def main():
    ...
    ds = pd.read_parquet(C.ARTIFACTS_DIR / "train_dataset_v2.parquet")
    ...
    for fold, (tr_idx, va_idx) in enumerate(gkf.split(t_rows, labels, groups)):
        model = train_fold_v2(fold, tr_idx, (t_rows, i_rows, labels), cache)

        oof_path = C.ARTIFACTS_DIR / f"ce_v2_oof_fold{fold}.npy"
        if oof_path.exists():
        ...
        pr_path = C.ARTIFACTS_DIR / f"ce_v2_proxy_fold{fold}.npy"
        if pr_path.exists():
    ...
    np.savez(C.ARTIFACTS_DIR / "ce_v2_oof.npz", oof=oof, y=labels,
             term_id=ds["term_id"].values, item_id=ds["item_id"].values,
             source=ds["source"].values)
    np.savez(C.ARTIFACTS_DIR / "ce_v2_proxy_scores.npz", scores=proxy_scores,
             y=proxy["label"].values, term_id=proxy["term_id"].values)

    eval_proxy.report("CE v2 OOF (gercek etiketli kisim)",
                      labels[real_mask], oof[real_mask])
    eval_proxy.report("CE v2 LB-proxy", proxy["label"].values, proxy_scores)
    # v1 ile dogrudan kiyas
